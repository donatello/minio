...
func (s ErasureStorage) HealFile(staleDisks []StorageAPI, volume, path string, blocksize int64,
	...
	numBlocks, lastBlockLen := ceilFrac(size, blocksize), size%blocksize
	lastChunkSize := getChunkSize(lastBlockLen, s.dataBlocks)
	readLen := chunksize*numBlocks + lastChunkSize
